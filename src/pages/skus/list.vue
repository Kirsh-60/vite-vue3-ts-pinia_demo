    ...
    .then((AMap: any) => {
      ...
      mouseTool.value.on('draw', (event: any) => {
        ...
        if (!isShapeValid(shape)) {
          ...
          showMessage('error', '绘制的图形不合规，请重新绘制！')
          return
        }
...
const isShapeValid = (shape: any) => {
  if (shape.CLASS_NAME === 'Overlay.Circle') {
    ...
    const isValidRectangle =
      path.length === 4 &&
      !path.every(
        (point: any) => point.lng === path[0].lng && point.lat === path[0].lat
      )
      ...
      if (shape.CLASS_NAME === 'Overlay.Circle') {
        ...
        return {
          ...
          data: {
            ...
            area: Number((shape.getArea() / 1000000).toFixed(4)),
          },
        }
      } else if (shape.CLASS_NAME === 'Overlay.Polygon') {
        return {
